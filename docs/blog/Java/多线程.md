...
![第三种方式：实现Callable接口(实现call方法)](./多线程/img-14.jpg)
...
sequenceDiagram
    ...
    Note over T: 线程t1开始运行
    
    M->>F: task.get()
    activate M
    Note over M: Main线程阻塞等待
    
    T->>C: 执行call()方法
    activate C
    Note over C: Thread.sleep(5000)<br/>模拟业务处理
    C-->>T: return 1
    deactivate C
...
>    - 设置线程名并启动线程
> 2. **执行阶段**：
>    - Main线程调用`task.get()`方法，**进入阻塞状态**，等待计算结果
...
sequenceDiagram
    ...
    Note over EP: 创建包含3个线程的线程池
    EP-->>M: 返回ExecutorService实例
    
    M->>ES: submit(Runnable Task)
    Note over ES: 接收任务并缓存
...
3.Lock 和 synchronized 哪个好？Lock更好。为什么？因为更加灵活。
```
...
sequenceDiagram
    ...
    Note over T1,T2: 初始状态: singleton = null
    
    T1->>Singleton: getSingleton()
    T2->>Singleton: getSingleton()
    
    Note over T1,T2: 两个线程几乎同时调用getSingleton()
    
    T1->>Singleton: if(singleton == null) ✓
    T2->>Singleton: if(singleton == null) ✓
    
    T1->>Lock: lock.lock()
    Note over T1,Lock: T1成功获取锁
    
    T2->>Lock: lock.lock()
    Note over T2,Lock: T2被阻塞，等待锁释放
    
    T1->>Singleton: if(singleton == null) ✓
    T1->>Singleton: Thread.sleep(2000)
    T1->>Singleton: new Singleton()
    Note over Singleton: 构造方法执行了！
    T1->>Singleton: singleton = new Singleton()
    
    T1->>Lock: lock.unlock()
    Note over T2,Lock: T2获取到锁，继续执行
...
```mermaid
sequenceDiagram
    participant T1 as t1线程
    participant Lock as obj锁
    participant T2 as t2线程
    participant Count as 共享count

    Note over T1,T2: 初始状态: count=0
    
    rect rgb(119, 136, 153)
        Note over T1: t1获取obj锁
        T1->>Lock: 进入synchronized块
        T1->>T2: obj.notify() (无效果)
        T1->>T1: Thread.sleep(50)
        T1->>Count: count++ (0→1)
        T1->>T1: 输出 "t1==>1"
        T1->>Lock: obj.wait() 释放锁
        Note over T1: t1进入WAITING状态
    end

    rect rgb(70, 130, 180)
        Note over T2: t2获取obj锁
        T2->>Lock: 进入synchronized块
        T2->>T1: obj.notify() 唤醒t1
        Note over T1: t1变为BLOCKED状态
        T2->>T2: Thread.sleep(50)
        T2->>Count: count++ (1→2)
        T2->>T2: 输出 "t2==>2"
        T2->>Lock: obj.wait() 释放锁
        Note over T2: t2进入WAITING状态
    end

    rect rgb(119, 136, 153)
        Note over T1: t1重新获取obj锁
        T1->>Lock: 进入synchronized块
        T1->>T2: obj.notify() 唤醒t2
        Note over T2: t2变为BLOCKED状态
        T1->>T1: Thread.sleep(50)
        T1->>Count: count++ (2→3)
        T1->>T1: 输出 "t1==>3"
        T1->>Lock: obj.wait() 释放锁
    ...
    Note over T1,T2: 如此循环直到count>=100
```
...
stateDiagram-v2
    ...
    state T1_线程 {
        RUNNABLE_t1 --> IN_SYNC_t1 : 获取obj锁
        IN_SYNC_t1 --> WAITING_t1 : obj.wait()
        WAITING_t1 --> BLOCKED_t1 : 被t2唤醒
        BLOCKED_t1 --> RUNNABLE_t1 : 重新获取锁
    }
    
    state T2_线程 {
        BLOCKED_t2 --> RUNNABLE_t2 : 获取obj锁
        RUNNABLE_t2 --> IN_SYNC_t2 : 进入同步块
        IN_SYNC_t2 --> WAITING_t2 : obj.wait()
        WAITING_t2 --> BLOCKED_t2 : 被t1唤醒
    }
...
```mermaid
sequenceDiagram
    participant T1 as Thread-1 (输出A)
    participant T2 as Thread-2 (输出B)
    participant T3 as Thread-3 (输出C)
    participant Lock as 共享锁对象

    Note over T1,T3: 初始状态: t1Output=true, t2Output=false, t3Output=false

    T1->>Lock: 获取锁(synchronized)
    T1->>T1: 检查t1Output=true，不等待
    T1->>T1: 输出"A"
    T1->>T1: 设置t1Output=false, t2Output=true, t3Output=false
    T1->>Lock: notifyAll()唤醒所有等待线程
    T1->>Lock: 释放锁(循环继续，重新获取锁)
    
    Note over T2,T3: 此时T2的t2Output=true，T1和T3的标记为false
    
    T2->>Lock: 获取锁成功
    T2->>T2: 检查t2Output=true，不等待
    T2->>T2: 输出"B"
    T2->>T2: 设置t1Output=false, t2Output=false, t3Output=true
    T2->>Lock: notifyAll()唤醒所有等待线程
    T2->>Lock: 释放锁(循环继续，重新获取锁)
    
    Note over T1,T3: 此时T3的t3Output=true，T1和T2的标记为false
    
    T3->>Lock: 获取锁成功
    T3->>T3: 检查t3Output=true，不等待
    T3->>T3: 输出"C"
    T3->>T3: 设置t1Output=true, t2Output=false, t3Output=false
    T3->>Lock: notifyAll()唤醒所有等待线程
    T3->>Lock: 释放锁(循环继续，重新获取锁)
    
    Note over T1,T3: 此时T1的t1Output=true，回到初始状态，开始新一轮循环
